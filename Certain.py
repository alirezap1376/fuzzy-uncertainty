...
demand = { k1:random.randint(10, 25) for k1 in k}
fixed_cost= {j1:random.randint(2000, 3500) for j1 in j }
transport_cost_ij = {(i1, j1):random.randint(1, 10) for i1 in i for j1 in j}
...
SecondTerm = sum( transport_cost_ij[i1,j1] * model.x[(i1,j1)] for i1 in i for j1 in j )
ThirdTerm = sum( transport_cost_jk[j1,k1] * model.u[(j1,k1)] for j1 in j for k1 in k)
FinalTerm = FirstTerm + SecondTerm + ThirdTerm

model.obj=py.Objective(expr= FinalTerm , sense = 1 )

model.constraint = ConstraintList()
for k1 in k:
    model.constraint.add( sum(model.u[(j1,k1)] for j1 in j) >= demand[k1]  )
for j1 in j:
